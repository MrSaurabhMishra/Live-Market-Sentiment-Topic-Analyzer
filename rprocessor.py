...
def run_ingestion():
    ...
    # Simulate slight confidence variation
          dynamic_confidence = res['confidence'] + random.uniform(-0.02, 0.02)
    # Ensure it stays within 0 and 1
          dynamic_confidence = max(0, min(1, dynamic_confidence))
    
    
          query = "INSERT INTO sentiment_logs (timestamp, ticker, sentiment, confidence, topic) VALUES (%s, %s, %s, %s, %s)"
          cur.execute(query, (datetime.now(), item['ticker'], res['sentiment'], dynamic_confidence, current_topic))
